...
function calcHeatScore(h: Hotspot): number {
  ...
  if (raw <= 0) return 0;
  // sqrt 压缩（比 log10 更平缓，小数值也能有可观的分数）
  return Math.min(100, Math.round(Math.sqrt(raw) * 10));
}

function getHeatLevel(score: number): { label: string; color: string } {
  if (score >= 75) return { label: '爆', color: 'text-red-400' };
  if (score >= 50) return { label: '热', color: 'text-orange-400' };
  if (score >= 30) return { label: '温', color: 'text-amber-400' };
  if (score >= 15) return { label: '凉', color: 'text-blue-400' };
...
function App() {
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0c] relative overflow-hidden">
      ...
      <Spotlight className="-top-40 left-0 md:left-60 md:-top-20" fill="#f0a028" />
      ...
      <header className="sticky top-0 z-40 backdrop-blur-2xl bg-[#0a0a0c]/70 border-b border-white/5">
                ...
                <AnimatePresence>
                  {showNotifications && (
                    <motion.div
                      ...
                      className="absolute right-0 top-14 w-80 bg-[#1a1a1e]/95 backdrop-blur-2xl rounded-2xl border border-white/10 shadow-2xl overflow-hidden"
                    >
                      <div className="flex items-center justify-between p-4 border-b border-white/5">
                        <h3 className="font-medium text-white">通知</h3>
                        {unreadCount > 0 && (
                          <button onClick={handleMarkAllRead} className="text-xs text-amber-400 hover:text-amber-300">
                            全部已读
                          </button>
                        )}
                      </div>
            ...
            {stats && (
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                <motion.div
                  ...
                  className="relative group p-5 rounded-2xl bg-[#141416] border border-amber-500/10 hover:border-amber-500/25 transition-all duration-500 overflow-hidden"
                >
                  {/* Crystal edge glow */}
                  <div className="absolute top-0 left-4 right-4 h-px bg-gradient-to-r from-transparent via-amber-500/30 to-transparent" />
                  <div className="absolute top-0 left-0 w-12 h-12 bg-amber-500/3 rounded-full blur-2xl group-hover:bg-amber-500/8 transition-colors" />
                  <div className="relative">
                    <div className="flex items-center gap-2 text-slate-500 text-xs font-medium mb-3 tracking-wider uppercase">
                      <Activity className="w-3.5 h-3.5 text-amber-400/70" />
                      总热点
                    </div>
                    <p className="text-4xl font-bold text-white tracking-tight tabular-nums">{stats.total}</p>
                  </div>
                </motion.div>
                ...
                <motion.div
                  ...
                  className="relative group p-5 rounded-2xl bg-[#141416] border border-blue-500/10 hover:border-blue-500/25 transition-all duration-500 overflow-hidden"
                >
                  <div className="absolute top-0 left-4 right-4 h-px bg-gradient-to-r from-transparent via-blue-500/30 to-transparent" />
                  <div className="absolute top-0 left-0 w-12 h-12 bg-blue-500/3 rounded-full blur-2xl group-hover:bg-blue-500/8 transition-colors" />
                  <div className="relative">
                    <div className="flex items-center gap-2 text-slate-500 text-xs font-medium mb-3 tracking-wider uppercase">
                      <Clock className="w-3.5 h-3.5 text-blue-400/70" />
                      今日新增
                    </div>
                    <p className="text-4xl font-bold text-blue-400 tracking-tight tabular-nums">{stats.today}</p>
                  </div>
                </motion.div>
                ...
                <motion.div
                  ...
                  className="relative group p-5 rounded-2xl bg-[#141416] border border-red-500/10 hover:border-red-500/25 transition-all duration-500 overflow-hidden"
                >
                  <Meteors number={4} />
                  <div className="absolute top-0 left-4 right-4 h-px bg-gradient-to-r from-transparent via-red-500/30 to-transparent" />
                  <div className="absolute top-0 left-0 w-12 h-12 bg-red-500/3 rounded-full blur-2xl group-hover:bg-red-500/8 transition-colors" />
                  <div className="relative">
                    <div className="flex items-center gap-2 text-slate-500 text-xs font-medium mb-3 tracking-wider uppercase">
                      <AlertTriangle className="w-3.5 h-3.5 text-red-400/70" />
                      紧急热点
                    </div>
                    <p className="text-4xl font-bold text-red-400 tracking-tight tabular-nums">{stats.urgent}</p>
                  </div>
                </motion.div>
                ...
                <motion.div
                  ...
                  className="relative group p-5 rounded-2xl bg-[#141416] border border-emerald-500/10 hover:border-emerald-500/25 transition-all duration-500 overflow-hidden"
                >
                  <div className="absolute top-0 left-4 right-4 h-px bg-gradient-to-r from-transparent via-emerald-500/30 to-transparent" />
                  <div className="absolute top-0 left-0 w-12 h-12 bg-emerald-500/3 rounded-full blur-2xl group-hover:bg-emerald-500/8 transition-colors" />
                  <div className="relative">
                    <div className="flex items-center gap-2 text-slate-500 text-xs font-medium mb-3 tracking-wider uppercase">
                      <Target className="w-3.5 h-3.5 text-emerald-400/70" />
                      监控词
                    </div>
                    <p className="text-4xl font-bold text-emerald-400 tracking-tight tabular-nums">{keywords.filter(k => k.isActive).length}</p>
                  </div>
                </motion.div>
              </div>
            )}
              ...
              {isLoading ? (
                ...
                <div className="space-y-3">
                  ...
                  {hotspots.some(h => h.relevanceReason) && (
                    <div className="flex justify-end">
                      <button
                        ...
                        className="flex items-center gap-1.5 text-xs text-slate-500 hover:text-amber-400 transition-colors px-3 py-1.5 rounded-lg hover:bg-white/5"
                      >
                  ...
                  {hotspots.map((hotspot, index) => {
                    ...
                    // 根据 importance 决定卡片左边框颜色
                    const importanceBorder = hotspot.importance === 'urgent' ? 'border-l-red-500/40'
                      : hotspot.importance === 'high' ? 'border-l-orange-500/30'
                      : hotspot.importance === 'medium' ? 'border-l-amber-500/20'
                    ...
                    <motion.div
                      ...
                      className={cn(
                        "group p-5 rounded-2xl bg-[#141416]/80 border border-white/5 hover:border-white/10 border-l-2 transition-all duration-300",
                        importanceBorder
                      )}
                    >
                      <div className="flex items-start justify-between gap-4">
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex flex-wrap items-center gap-2 mb-3">
                            {/* Importance + Source in one capsule */}
                            <span className={cn(
                              "inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-[10px] font-semibold uppercase tracking-wider",
                              hotspot.importance === 'urgent' && "bg-red-500/10 text-red-400 border border-red-500/20",
                              hotspot.importance === 'high' && "bg-orange-500/10 text-orange-400 border border-orange-500/20",
                              hotspot.importance === 'medium' && "bg-amber-500/10 text-amber-400 border border-amber-500/20",
                              hotspot.importance === 'low' && "bg-emerald-500/10 text-emerald-400 border border-emerald-500/20"
                            )}>
                              {getImportanceIcon(hotspot.importance)}
                              <span>{hotspot.importance}</span>
                            </span>
                            {/* Source */}
                            <span className="inline-flex items-center gap-1 text-[11px] text-slate-500 bg-white/[0.03] px-2 py-1 rounded-lg border border-white/5">
                            ...
                            {/* Credibility badge */}
                            {!hotspot.isReal && (
                              <span className="inline-flex items-center gap-1 text-[10px] px-2 py-0.5 rounded-md bg-red-500/10 text-red-400 border border-red-500/20">
                            ...
                            {hotspot.keywordMentioned === true && (
                              <span className="inline-flex items-center gap-1 text-[10px] px-2 py-0.5 rounded-md bg-purple-500/10 text-purple-400 border border-purple-500/20">
                                <Target className="w-3 h-3" />
                                直接提及
                              </span>
                            )}
                            {hotspot.keywordMentioned === false && (
                              <span className="inline-flex items-center gap-1 text-[10px] px-2 py-0.5 rounded-md bg-yellow-500/10 text-yellow-500 border border-yellow-500/20">
                                <Target className="w-3 h-3" />
                                间接相关
                              </span>
                            )}
                          ...
                          <h3 className="font-semibold text-[15px] text-white mb-3 leading-relaxed group-hover:text-amber-400 transition-colors line-clamp-2">
                            {hotspot.title}
                          </h3>
                          ...
                          {hotspot.summary && (
                            <div className="mb-3 flex items-start gap-2">
                              <span className="shrink-0 text-[10px] text-amber-400/50 font-medium mt-0.5">AI</span>
                              <span className="text-[13px] text-slate-400 leading-relaxed">{hotspot.summary}</span>
                            </div>
                          )}
                          ...
                          <div className="flex flex-wrap items-center gap-x-4 gap-y-1 mb-2">
                            {hotspot.authorName && (
                              <div className="flex items-center gap-1.5">
                                {hotspot.authorAvatar ? (
                                  <img src={hotspot.authorAvatar} alt="" className="w-4 h-4 rounded-full object-cover" />
                                ) : (
                            ...
                            {/* Relevance + Heat combined */}
                            <span className="inline-flex items-center gap-1 text-xs text-slate-500">
                              <Target className="w-3 h-3" />
                              相关 {hotspot.relevance}%
                            </span>
                            <span className={cn("inline-flex items-center gap-1 text-xs font-medium", heat.color)}>
                          ...
                          <div className="flex flex-wrap items-center gap-3 text-[11px] text-slate-600">
                            ...
                            {hotspot.retweetCount != null && hotspot.retweetCount > 0 && (
                              <span className="inline-flex items-center gap-1" title="转发">
                            ...
                            {hotspot.quoteCount != null && hotspot.quoteCount > 0 && (
                              <span className="inline-flex items-center gap-1" title="引用">
                            ...
                            {hotspot.danmakuCount != null && hotspot.danmakuCount > 0 && (
                              <span className="inline-flex items-center gap-1" title="弹幕">
                          ...
                          <div className="flex flex-wrap items-center gap-3 mt-2 text-[11px] text-slate-600">
                            ...
                            <span className="inline-flex items-center gap-1" title={formatDateTime(hotspot.createdAt)}>
                            ...
                            <div className="mt-2">
                              ...
                              <AnimatePresence>
                                {expandedReasons.has(hotspot.id) && (
                                  ...
                                  >
                                    <p className="text-xs text-slate-500 mt-1 pl-4 border-l-2 border-amber-500/20">
                                      {hotspot.relevanceReason}
                                    </p>
                                  </motion.div>
                                )}
                        ...
                        <a
                          ...
                          className="p-2.5 rounded-xl bg-white/5 hover:bg-amber-500/20 text-slate-500 hover:text-amber-400 transition-all opacity-0 group-hover:opacity-100"
                        >
              ...
              <div className="flex gap-3">
                <div className="flex-1 relative">
                  <input
                    ...
                    className="w-full px-4 py-3 rounded-xl bg-white/5 border border-white/10 text-white placeholder-slate-600 focus:outline-none focus:border-amber-500/50 focus:ring-2 focus:ring-amber-500/20 transition-all"
                  />
                </div>
                <motion.button 
                  type="submit" 
                  whileHover={{ scale: 1.02 }}
                  whileTap={{ scale: 0.98 }}
                  className="px-6 py-3 rounded-xl bg-gradient-to-r from-amber-500 to-amber-600 text-white font-medium flex items-center gap-2 shadow-lg shadow-amber-500/25"
                >
                ...
                {keywords.map((keyword, i) => (
                  <motion.div
                    ...
                    className={cn(
                      ...
                      keyword.isActive 
                        ? "bg-white/[0.03] border-amber-500/20 hover:border-amber-500/30" 
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <button
                          ...
                          className={cn(
                            ...
                            keyword.isActive ? "bg-amber-500" : "bg-slate-700"
                          )}
                        >
                          <span className={cn(
              ...
              <div className="flex gap-3">
                <div className="flex-1 relative">
                  ...
                  <input
                    ...
                    placeholder="搜索热点内容..."
                    className="w-full pl-12 pr-4 py-3 rounded-xl bg-white/5 border border-white/10 text-white placeholder-slate-600 focus:outline-none focus:border-amber-500/50 focus:ring-2 focus:ring-amber-500/20 transition-all"
                  />
                </div>
                <motion.button 
                  type="submit" 
                  ...
                  whileHover={{ scale: 1.02 }}
                  whileTap={{ scale: 0.98 }}
                  className="px-6 py-3 rounded-xl bg-gradient-to-r from-amber-500 to-amber-600 text-white font-medium flex items-center gap-2 shadow-lg shadow-amber-500/25 disabled:opacity-50"
                >
                  {isLoading ? (
            ...
            <div className="space-y-3">
              ...
              {filteredSearchResults.map((hotspot, i) => {
                ...
                <motion.div
                  ...
                  className="group p-5 rounded-2xl bg-[#141416]/80 border border-white/5 hover:border-white/10 border-l-2 border-l-amber-500/20 transition-all duration-300"
                >
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1 min-w-0">
                      <div className="flex flex-wrap items-center gap-2 mb-3">
                        ...
                        <span className={cn("inline-flex items-center gap-1 text-[10px] px-2 py-0.5 rounded-md border font-medium", heat.color, "bg-white/5 border-white/10")}>
                          <ThermometerSun className="w-3 h-3" />
                          {heat.label} {heatScore}
                        </span>
                      </div>
                      <h3 className="font-semibold text-[15px] text-white mb-3 leading-relaxed group-hover:text-amber-400 transition-colors line-clamp-2">{hotspot.title}</h3>
                      {hotspot.summary && (
                      ...
                      <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-slate-500">
                        ...
                        {hotspot.likeCount != null && hotspot.likeCount > 0 && (
                          <span className="inline-flex items-center gap-1"><Zap className="w-3 h-3" />{hotspot.likeCount.toLocaleString()}</span>
                        )}
                        {hotspot.viewCount != null && hotspot.viewCount > 0 && (
                          <span className="inline-flex items-center gap-1"><Eye className="w-3 h-3" />{hotspot.viewCount.toLocaleString()}</span>
                        )}
                      </div>
                      {hotspot.publishedAt && (
                    ...
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="shrink-0 px-4 py-2 rounded-xl bg-amber-500/10 hover:bg-amber-500/20 text-amber-400 text-sm font-medium transition-all"
                    >
                      查看
                    </a>
                  </div>
